 ...
 * Provides mute and gag functionality using SourceComms
 *
...
void DisplayVoteMuteMenu(int client, int target)
{
	...
	Action result = Forward_OnVoteInitiated(client, SBPP_VoteType_Mute, target, g_voteInfo[VOTE_NAME], g_voteArg);
	...
	ShowActivity2(client, "[SM] ", "%t", "Initiated Vote Mute", g_voteInfo[VOTE_NAME]);
	...
	Forward_OnVoteInitiated_Post(client, SBPP_VoteType_Mute, target, g_voteInfo[VOTE_NAME], g_voteArg);
}

void DisplayMuteTargetMenu(int client)
{
	Menu menu = new Menu(MenuHandler_Mute);
	
	char title[100];
	Format(title, sizeof(title), "%T:", "Mute vote", client);
	menu.SetTitle(title);
...
public void AdminMenu_VoteMute(TopMenu topmenu, 
							  ...
							  int maxlength)
{
	...
	{
		Format(buffer, maxlength, "%T", "Mute vote", param);
	}
...
void DisplayVoteGagMenu(int client, int target)
{
	...
	// Call OnVoteInitiated forward
	Action result = Forward_OnVoteInitiated(client, SBPP_VoteType_Gag, target, g_voteInfo[VOTE_NAME], g_voteArg);
	if (result >= Plugin_Handled)
	{
		return;
	}
	...
	ShowActivity2(client, "[SM] ", "%t", "Initiated Vote Gag", g_voteInfo[VOTE_NAME]);
	...
	// Call OnVoteInitiated_Post forward
	Forward_OnVoteInitiated_Post(client, SBPP_VoteType_Gag, target, g_voteInfo[VOTE_NAME], g_voteArg);
}

void DisplayGagTargetMenu(int client)
{
	Menu menu = new Menu(MenuHandler_Gag);
	
	char title[100];
	Format(title, sizeof(title), "%T:", "Gag vote", client);
	menu.SetTitle(title);
	menu.ExitBackButton = true;
...
public void AdminMenu_VoteGag(TopMenu topmenu, 
							  TopMenuAction action,
							  TopMenuObject object_id,
							  int param,
							  char[] buffer,
							  int maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
		Format(buffer, maxlength, "%T", "Gag vote", param);
	}
	else if (action == TopMenuAction_SelectOption)
	{
